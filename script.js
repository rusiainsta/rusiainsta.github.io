document.addEventListener("DOMContentLoaded", () => {
  ...
  // Slider işlevselliği - Dokunmatik kaydırma desteği ile
    ...
    sliders.forEach((slider) => {
      ...
      sliderElement.addEventListener(
        ...
        (e) => {
          ...
          // Kaydırma animasyonunu devre dışı bırak ve anlık pozisyon değişikliği yap
          sliderElement.style.transition = "none"
          sliderElement.style.transform = `translateX(calc(-${currentIndex * 100}% + ${diff}px))`
        },
        { passive: true },
      )
      ...
      sliderElement.addEventListener(
        ...
        (e) => {
          ...
          // Kaydırma animasyonunu tekrar etkinleştir
  ...
  // Tam ekran slider'ı ayarla - Dokunmatik kaydırma desteği ile
  function setupFullscreenSlider(product, initialIndex) {
    ...
    // Dokunmatik kaydırma olayları
    fullscreenSliderContainer.addEventListener(
      "touchstart",
      (e) => {
        startX = e.touches[0].clientX
        isDragging = true
    ...
    fullscreenSliderContainer.addEventListener(
      ...
      (e) => {
        ...
        // Kaydırma animasyonunu devre dışı bırak ve anlık pozisyon değişikliği yap
        fullscreenSliderContainer.style.transition = "none"
        fullscreenSliderContainer.style.transform = `translateX(calc(-${currentFullscreenIndex * 100}% + ${diff}px))`
      },
      { passive: true },
    )
    ...
    fullscreenSliderContainer.addEventListener(
      ...
      (e) => {
        ...
        // Kaydırma animasyonunu tekrar etkinleştir
        fullscreenSliderContainer.style.transition = "transform 0.3s ease"
        ...
        if (Math.abs(diff) > threshold) {
          if (diff > 0) {
            // Sağa kaydırma - önceki resim
            currentFullscreenIndex = (currentFullscreenIndex - 1 + product.images.length) % product.images.length
          } else {
            // Sola kaydırma - sonraki resim
            currentFullscreenIndex = (currentFullscreenIndex + 1) % product.images.length
          }
        }
    ...
    // Fare ile kaydırma olayları
    fullscreenSliderContainer.addEventListener("mousedown", (e) => {
      startX = e.clientX
      isDragging = true
      e.preventDefault()
    })

    fullscreenSliderContainer.addEventListener("mousemove", (e) => {
      if (!isDragging) return
      moveX = e.clientX
      const diff = moveX - startX
    ...
    fullscreenSliderContainer.addEventListener("mouseup", (e) => {
      if (!isDragging) return
      isDragging = false
    ...
    fullscreenSliderContainer.addEventListener("mouseleave", () => {
      if (isDragging) {
        isDragging = false
        fullscreenSliderContainer.style.transition = "transform 0.3s ease"
        updateFullscreenSlider()
  ...
  // Sayfa yüklendiğinde hiçbir tab'ı otomatik olarak gösterme
  // İlk tab'ı gösterme kodu kaldırıldı
})
